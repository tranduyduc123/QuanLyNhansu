...
var addStaffMoves=document.querySelectorAll('.add-staff-item');
 ...
 //ckeck table
     ...
     function checkEmail2(email){
      ...
      listStaffs.forEach(function(stt){
        if(stt.email==email.value){
           checkEmail2xxx=true;
           showError(email,'thông tin đã tồn tại') 
        }
      })
      return checkEmail2xxx;
    }
     ...
     function checkSdt2(sdt){
      let checkSdt2xxx=false;
      let listStaffs=localStorage.getItem("list-staffs") ? JSON.parse(localStorage.getItem("list-staffs")) :[];
     
      listStaffs.forEach(function(stt){
        if(stt.sdt==sdt.value){
            checkSdt2xxx=true;
           showError(sdt,'thông tin đã tồn tại') 
        }
      })
      return checkSdt2xxx;
    }
     ...
     function checkFullname(fullname){
       let checkFullnamexxx=false;
       let listStaffs=localStorage.getItem("list-staffs") ? JSON.parse(localStorage.getItem("list-staffs")) :[];
      
       listStaffs.forEach(function(stt){
         console.log(stt.fullname)
         console.log(fullname.value)
         if(stt.fullname==fullname.value){
            console.log('ducdz')
            checkFullnamexxx=true;
            showError(fullname,'thông tin đã tồn tại') 
         }
       })
       return checkFullnamexxx;
     }
     var gender=" ";
 ...
 var genderNu=document.querySelector('.board-Staff-item #famale')
      ...
      let checkFullnamexxx=checkFullname(fullname)
      let checkEmailXXX= checkEmail(email)
       let checkSdtXXX=checkSdt(sdt)
       let checkCmndXXX=checkCmnd(cmnd)
       let checkEmail2xxx=checkEmail2(email)
       let checkSdt2xxx=checkSdt2(sdt)
       checkSeed(checkSeeds)
       checkWord(checkWords)
       if(checkSdt2xxx || checkEmail2xxx || checkFullnamexxx || checkemtyErrorXXX || checkEmailXXX || checkSdtXXX || checkCmndXXX){
      //      console.log('ducdz') 
